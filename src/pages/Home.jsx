...
const SectionLabel = ({ children }) => (
  <div className="mb-6 flex items-center gap-3 font-jbmono text-[12px] uppercase tracking-[0.25em]" style={{ color: T.signal }}>
    <span className="h-1.5 w-1.5" style={{ background: T.signal }} />{children}
  </div>
);
  ...
  return (
    <div style={{ background: T.bg, color: T.text }} className="font-archivo">
      ...
      <section className="grain relative min-h-screen overflow-hidden">
        <img src="https://images.unsplash.com/photo-1607694418429-5cfe3b73bb35?crop=entropy&cs=srgb&fm=jpg&q=85&w=1600" alt="" className="pointer-events-none absolute right-0 top-0 h-full w-full object-cover opacity-[0.22] md:w-2/3" style={{ maskImage: "linear-gradient(90deg,transparent,#000 65%)", WebkitMaskImage: "linear-gradient(90deg,transparent,#000 65%)" }} />
        <div className="hairline-grid absolute inset-0" style={{ "--hl": T.hl, backgroundSize: "8.33% 6rem" }} />
        <div className="pointer-events-none absolute -left-40 top-1/3 h-[520px] w-[520px] rounded-full" style={{ background: T.signal, filter: "blur(220px)", opacity: 0.1 }} />
        <div className="relative mx-auto flex min-h-screen max-w-[1400px] flex-col justify-center px-6 pt-28 pb-20 md:px-12">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.8 }} className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <Link to="/solutions" className="inline-flex items-center gap-2 border px-8 py-4 text-sm font-semibold uppercase tracking-wider transition-colors hover:border-[#5CF0CE]" style={{ borderColor: "rgba(238,241,244,0.2)" }}>
              View All Services
            </Link>
          </motion.div>
        </div>
              ...
              <Reveal key={s.name} delay={i * 0.05}>
                <Link to={s.to} data-testid={`service-card-${i}`} className="group flex h-full flex-col justify-between p-8 transition-colors" style={{ background: T.panel }}>
                  <div>
                    ...
                    <h3 className="mt-4 text-xl font-semibold group-hover:text-[#5CF0CE]" style={{ transition: "color .2s" }}>{s.name}</h3>
            ...
            {VALUE_PROPS.map((v, i) => (
              <Reveal key={v.t} delay={i * 0.05}>
                <div className="flex h-full items-start gap-4 rounded-lg border p-6" style={{ borderColor: T.border, background: T.panel }}>
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md" style={{ background: "rgba(92,240,206,0.12)", color: T.signal }}>
                    <v.icon size={20} />
                  </div>
          ...
          <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-2">
            {DEPLOY.map((d, i) => (
              <Reveal key={d.t} delay={i * 0.08}>
                <div className="group flex h-full flex-col rounded-lg border p-10 transition-colors hover:border-[#5CF0CE]" style={{ borderColor: T.border, background: T.panel }}>
                  <div className="flex h-14 w-14 items-center justify-center rounded-md" style={{ background: "rgba(92,240,206,0.12)", color: T.signal }}><d.icon size={26} /></div>
          ...
          <div className="lg:col-span-7">
            <Reveal delay={0.1}>
              ...
              >
                ...
                <input name="name" required placeholder="Name" data-testid="contact-name" className="rounded-md border bg-transparent px-4 py-3.5 text-sm outline-none focus:border-[#5CF0CE]" style={{ borderColor: T.border, color: T.text }} />
                <input name="email" type="email" required placeholder="Email" data-testid="contact-email" className="rounded-md border bg-transparent px-4 py-3.5 text-sm outline-none focus:border-[#5CF0CE]" style={{ borderColor: T.border, color: T.text }} />
                <input name="company" placeholder="Company (optional)" data-testid="contact-company" className="rounded-md border bg-transparent px-4 py-3.5 text-sm outline-none focus:border-[#5CF0CE] sm:col-span-2" style={{ borderColor: T.border, color: T.text }} />
                <textarea name="message" required rows={4} placeholder="How may we help you?" data-testid="contact-message" className="rounded-md border bg-transparent px-4 py-3.5 text-sm outline-none focus:border-[#5CF0CE] sm:col-span-2" style={{ borderColor: T.border, color: T.text }} />
