              ...
              <Reveal key={post.slug} delay={i * 0.06}>
                <Link to={`/blog/${post.slug}`} data-testid={`blog-card-${i}`} className="group flex h-full flex-col justify-between p-10 transition-colors" style={{ background: T.panel }}>
                  <div>
                    ...
                    <h2 className="mt-4 text-2xl font-semibold leading-snug group-hover:text-[#5CF0CE]" style={{ transition: "color .2s" }}>{post.title}</h2>
                    {post.titleFlag && (
                      // NEEDS VERIFICATION — visible reviewer note (remove once title confirmed)
                      <p className="mt-3 rounded border px-3 py-1.5 font-jbmono text-[10px] uppercase tracking-wider" style={{ borderColor: "rgba(92,240,206,0.35)", color: T.faint }}>⚑ {post.titleFlag}</p>
                    )}
