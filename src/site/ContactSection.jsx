    ...
    <section className="relative overflow-hidden border-t px-6 py-24 md:px-12" style={{ borderColor: T.border, background: T.bg }} aria-labelledby="contact-section-heading">
      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-14 lg:grid-cols-12">
          ...
          <Reveal delay={0.1}>
            <div className="mt-8 space-y-6 text-sm" style={{ color: T.muted }}>
              ...
              <div className="space-y-1 font-jbmono">
                ...
                <div><a href="tel:+966555203079" className="rounded-sm hover:text-white">+966 555203079</a> · <a href="tel:+97143997558" className="rounded-sm hover:text-white">+971 43997558</a></div>
              </div>
            </div>
          </Reveal>
        </div>
        <div className="lg:col-span-7">
          <Reveal delay={0.1}>
            <form data-testid="contact-form" onSubmit={handleSubmit} aria-busy={sending} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="shared-contact-name" className="sr-only">Name</label>
                <input id="shared-contact-name" name="name" required autoComplete="name" placeholder="Name" data-testid="cs-name" className={`${fieldClass} w-full`} style={{ borderColor: T.border, color: T.text }} />
              </div>
              <div>
                <label htmlFor="shared-contact-email" className="sr-only">Email address</label>
                <input id="shared-contact-email" name="email" type="email" required autoComplete="email" inputMode="email" placeholder="Email" data-testid="cs-email" className={`${fieldClass} w-full`} style={{ borderColor: T.border, color: T.text }} />
              </div>
              <div className="sm:col-span-2">
                <label htmlFor="shared-contact-company" className="sr-only">Company</label>
                <input id="shared-contact-company" name="company" autoComplete="organization" placeholder="Company (optional)" data-testid="cs-company" className={`${fieldClass} w-full`} style={{ borderColor: T.border, color: T.text }} />
              </div>
              <div className="sm:col-span-2">
                <label htmlFor="shared-contact-message" className="sr-only">How may we help you?</label>
                <textarea id="shared-contact-message" name="message" required rows={4} placeholder="How may we help you?" data-testid="cs-message" className={`${fieldClass} w-full`} style={{ borderColor: T.border, color: T.text }} />
              </div>
